...
### set grid size here  
size <- 1000
  p <- spsample(WNA, cellsize = c(size), type = "hexagonal",offset = c(0, 0))# change this to change grid size and type regular (need 2 sizes)
...
pts.info <- left_join(pnt_state, coords, by = "ID1")
 # overlay points over BGC boundaries
   #  coords$BGC <- over(p,)
write.csv (pts.info,  "./inputs/created/WNA_1km_Grid.csv", row.names = FALSE) 
 pts.spatial <- SpatialPointsDataFrame(pts.info[,3:4],
                    pts.info,    #the R object to convert
                    proj4string = CRS.NAD83)   # assign a CRS 

  writeOGR (obj = pts.spatial, dsn = "./inputs/created/WNA_HexGrid_1k.shp", layer = "GridPoints", driver = "ESRI Shapefile", overwrite_layer = TRUE)

```

##Process for adding BGC to grid points
