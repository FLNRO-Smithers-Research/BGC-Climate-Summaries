...
mydata_spatial <-  st_as_sf (mydata2, coords = c("Longitude", "Latitude"), crs = CRS.NAD83) %>% st_transform(CRS.albers)
...
var.list <- c("BGC", "CMD") # for SI50 calculations
#mydata3 <- select (mydata2, var.list) 

mydata6 <- mydata5 %>% dplyr:: select(var.list, starts_with("DD5"), starts_with("DD18"), starts_with("Tmax")) %>% drop_na()
